...
interface ItemInput {
  ...
  priceFloat: string; // for user input representation
}
...
export default function CreateInvoicePage() {
  ...
  const [taxRateBps, setTaxRateBps] = useState(1800); // 18% default Bps
  ...
  useEffect(() => {
    ...
    const payload = {
      customerId: selectedCustomerId || '600000000000000000000001', // placeholder if none selected yet for preview
      invoiceDate,
      items: items.map((it) => ({
  ...
  async function handleSaveDraft(e: React.FormEvent) {
    ...
    try {
      await apiClient.createInvoiceDraft(payload);
      router.push('/dashboard/invoices');
    } catch (err: any) {
      setErrorMsg(err.message || 'Failed to save draft invoice');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary">New Invoice Draft</h1>
        <p className="text-sm text-text-secondary mt-1">
          Draft a new bill. System will calculate line subtotals, tax fractions, and round grand totals on draft save.
        </p>
      </div>
          ...
          <div className="bg-surface-app border border-border-app p-6 rounded-xl shadow-sm space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Special remarks, signature instructions, banking default terms..."
                className="w-full px-3 py-2 bg-surface-app border border-border-app rounded-lg text-sm text-text-primary"
                rows={3}
              />
            </div>
